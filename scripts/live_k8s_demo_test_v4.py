...
df_full = pd.read_csv("data/telecom_fleet_two_weeks_demo.csv", parse_dates=["timestamp"])
df = df_full.copy()
...
# Build per-machine sorted arrays for rolling-history lookup. For each sampled
# row we need the 10 consecutive readings (that machine, ending at that
# timestamp) to compute rolling features exactly as training did.
print("Building per-machine history index for rolling features...")
ROLLING_COLS = ["cpu", "ram", "load_avg"]
WINDOW = 10
machine_series = {}
for machine, grp in df_full.sort_values("timestamp").groupby("machine"):
    machine_series[machine] = grp.reset_index(drop=True)
# Map (machine, timestamp) -> integer position, for fast slicing
machine_ts_pos = {}
for machine, grp in machine_series.items():
    machine_ts_pos[machine] = {ts: i for i, ts in enumerate(grp["timestamp"])}
print(f"  Indexed {len(machine_series)} machines")

def build_history(machine, timestamp):
    """Return {'cpu':[...], 'ram':[...], 'load_avg':[...]} of up to 10 readings
    ending at timestamp (inclusive), or None if we can't build a full window."""
    grp = machine_series.get(machine)
    if grp is None:
        return None
    pos = machine_ts_pos[machine].get(timestamp)
    if pos is None or pos < WINDOW - 1:
        return None  # not enough history before this point
    lo = pos - WINDOW + 1
    window = grp.iloc[lo:pos + 1]
    return {c: [float(v) for v in window[c].values] for c in ROLLING_COLS}
...
def call_predict(row):
    ...
    hist = build_history(row["machine"], row["timestamp"])
    if hist is not None:
