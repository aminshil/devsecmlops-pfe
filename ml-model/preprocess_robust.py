...
def build_baselines(df, feature_cols,
    ...
    def robust_mean_std(series):
        """Trim top/bottom 5% before computing mean/std, so injected
        anomalies in the training split don't skew the baseline they're
        meant to help detect against."""
        lo, hi = series.quantile(0.05), series.quantile(0.95)
        trimmed = series[(series >= lo) & (series <= hi)]
        if len(trimmed) < 10:
            trimmed = series
        return float(trimmed.mean()), float(trimmed.std(ddof=0))

    baselines = {}

    # 1. Per-machine per-window baselines (primary) -- ROBUST (trimmed)
    for (machine, window), group in df.groupby([machine_col, window_col]):
        key = f"{machine}|{window}"
        baselines[key] = {}
        for col in feature_cols:
            mean, std = robust_mean_std(group[col])
