...
# Novelty detection: train ONLY on confirmed-normal rows.
# Test set stays a realistic mix (both normal and anomalous), same as always.
df_tr_full, df_te = train_test_split(df, test_size=0.3, random_state=42, stratify=df["label"])
df_tr = df_tr_full[df_tr_full["label"] == 0].copy()
print(f"  Novelty mode: training on {len(df_tr):,} normal-only rows "
     f"(excluded {len(df_tr_full) - len(df_tr):,} anomalous rows from training)")
...
joblib.dump(model, MODELS / "telecom_serving_model_novelty.pkl")
save_baselines(baselines, MODELS / "telecom_serving_baselines_novelty.json")
